...
void lu_openmp_loop(Matrix& A, int block_size) {
    if (block_size <= 0) {
    ...
    for (int k = 0; k < n; k += block_size) {
        ...
        // Factorize the diagonal block (SEQUENTIAL)
        // Data dependencies here are too tight to efficiently parallelize.
        ...
        // Compute L panel (PARALLEL)
        // Each row 'i' is completely independent.
